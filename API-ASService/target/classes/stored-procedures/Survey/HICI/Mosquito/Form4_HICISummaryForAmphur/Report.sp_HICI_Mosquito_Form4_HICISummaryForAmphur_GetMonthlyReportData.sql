...
-- =============================================
ALTER PROCEDURE Report.sp_HICI_Mosquito_Form4_HICISummaryForAmphur_GetMonthlyReportData
...
BEGIN
	...
	FROM dbo.Tumbol AS tbTumbol 
		LEFT JOIN Amphur AS tbAmphur ON tbTumbol.AmphurCode = tbAmphur.Code
	...
	AND (
		tbAmphur.Code = ISNULL(NULLIF(@AmphurCode,''), tbAmphur.Code)								-- Amphur
		OR (NULLIF(@AmphurCode, '') IS NULL) AND tbAmphur.Code IS NULL
	)
	AND (																							-- Month
		(CASE
			...
			/*
			WHEN (@Month >= 1 AND (@Month >= MONTH(tbSurveyHeader.StartDate) AND @Month <= MONTH(tbSurveyHeader.EndDate))) THEN 1
			WHEN (@Month >= 1 AND (@Month < MONTH(tbSurveyHeader.StartDate) OR @Month > MONTH(tbSurveyHeader.EndDate))) THEN 0
			*/
		END) = 1
	)
	AND (																							-- Year
